...
(*             cvg_expr == convergence of z ^+ n with 0 <= z < 1              *)
(*   geometric_seq_invn == 1 / k ^ n (1 < k)                                  *)
(* geometric_series_cvg == the geometric series converges                     *)
(*       exp_cvg_series == convergence of \sum_n^+oo x^n / n!                 *)
(*                e_seq == (1 + 1 / n) ^ n                                    *)
...
Lemma arithmetic_dvg (a : R) (z : R^o) : z > 0 -> arithmetic a z --> +oo.
Proof.
...
Lemma cvg_expr (z : R) : 0 <= z < 1 -> (fun n => z ^+ n : R^o) --> (0 : R^o).
Proof.
...
apply (@squeeze _ _ (fun=> (0 : R^o)) _ (fun n => t^-1 * harmonic n)).
- near=> n.
...
- rewrite (_ : 0 = t^-1 *: 0); last by rewrite scaler0.
  rewrite (_ : (fun _ => _) = (fun n => t^-1 *: harmonic n)); last first.
    by rewrite funeqE.
  apply: (@cvgZ _ [normedModType R of R^o] nat_topologicalType).
  exact: (@cvg_cst [topologicalType of R^o]).
- exact: cvg_harmonic.
Grab Existential Variables. all: end_near. Qed.

Definition geometric_invn x : R^o ^nat := fun n => (x ^ n)%:R^-1.

Lemma cvg_geometric_invn m :
  (1 < m)%nat -> geometric_invn m --> (0 : R^o).
Proof.
move=> m1.
rewrite /geometric_invn.
...
apply: (@mulIr _ (1 - z)) => //.
rewrite -!mulrA mulVr // mulr1 /= /series/=.
elim: n  => [|n ih].
...
rewrite /series; apply/cvg_ex; exists (a * (1 - z)^-1).
...
have ea1z_gt0 : (0%R < (e)%:num / `|a / (1 - z)|)%O.
  ...
  by rewrite gt_eqF.
move: (@cvg_expr z); rewrite (ltW z0) z1 => /(_ isT).
...
Lemma exp_cvg_series_pos : 0 < x -> cvg (series (exp_ x)).
Proof.
...
by apply: exp_cvg_series_pos; rewrite normr_gt0.
Grab Existential Variables. all: end_near. Qed.
...
apply: (@near_nondecreasing_is_cvg _ _ 4%:R).
  by apply: locallyW => x y ?; rewrite increasing_e_seq.
...
have K : (fun n : nat => ratr (1%:~R / (2 ^ n)%:~R) : R^o) --> (0 : R^o).
  rewrite (_ : (fun _ => _) = geometric_invn R 2); last first.
    rewrite funeqE => n; rewrite /geometric_invn /ratr.
...
Lemma harmonic_dvg (R : realType) : ~ cvg (series (@harmonic R)).
Proof.
...
rewrite sub_series_geq; last by near: m; apply: locally_infty_ge.
...
Lemma riemann_dvg (a : R): 0 < a <= 1 -> ~ cvg (series (riemann a)).
Proof.
case/andP => a0; rewrite le_eqVlt => /orP[/eqP ->|a1].
  rewrite (_ : riemann 1 = harmonic); first exact: harmonic_dvg.

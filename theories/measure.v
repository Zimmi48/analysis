...
Lemma sub_bigUB_of (A : (set T) ^nat) n :
...
Lemma A_B_of (A : (set T) ^nat) : {homo A : n m / (n <= m)%nat >-> n `<=` m} ->
  \bigcup_n A n = \bigcup_n (B_of A) n.
Proof.
move=> ndA; rewrite funeqE => x; rewrite propeqE; split.
  move=> -[n _]; rewrite (sub_bigUB_of _ ndA) bigcup_ord => -[m mn ?].
  by exists m.
move=> -[m _] myBAmx; exists m => //; rewrite (sub_bigUB_of _ ndA) bigcup_ord.
by exists m.
Qed.
...
have AE n : A n = \big[setU/set0]_(i < n.+1) (B_of A) i := sub_bigUB_of n ndA.
